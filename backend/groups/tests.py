...
def member_in_group(user, group, another_user):
    Membership.objects.create(
        user=another_user, group=group, role=Membership.Role.MEMBER
    )
    return another_user
...
def admin_in_group(user, group, another_user):
    Membership.objects.create(
        user=another_user, group=group, role=Membership.Role.ADMIN
    )
    return another_user
...
class TestSignalCreatePersonalGroup:
    ...
    def test_personal_group_only_created_once(self):
        user = User.objects.create_user(
            username="uniqueuser", email="unique@example.com", password="password123"
        )
...
class TestMembershipModel:
    def test_membership_creation(self, user, group_no_membership):
        membership = Membership.objects.create(
            user=user, group=group_no_membership, role=Membership.Role.ADMIN
        )
...
class TestInvitationModel:
    def test_invitation_creation(self, group, user, another_user):
        invitation = Invitation.objects.create(
            group=group, invited_user=another_user, invited_by=user
        )
...
class TestInvitePermissions:
    def test_invite_as_head_admin_success(
        self, authenticated_client, group, user, another_user
    ):
        response = authenticated_client.post(
            f"/api/groups/{group.id}/invite/", {"username": "anotheruser"}
        )
        assert response.status_code == 201
        assert Invitation.objects.filter(
            invited_user=another_user, group=group
        ).exists()

    def test_invite_as_member_fails(
        self, api_client, group, user, another_user, member_in_group
    ):
        token = Token.objects.get_or_create(user=member_in_group)[0]
        api_client.credentials(HTTP_AUTHORIZATION=f"Token {token.key}")
        User.objects.create_user(username="thirduser", password="pass")

        response = api_client.post(
            f"/api/groups/{group.id}/invite/", {"username": "thirduser"}
        )
        assert response.status_code == 403

    def test_invite_banned_user_fails(
        self, authenticated_client, group, user, another_user
    ):
        Ban.objects.create(group=group, user=another_user, banned_by=user)
        response = authenticated_client.post(
    ...
    def test_invite_already_member_fails(
        self, authenticated_client, group, user, another_user
    ):
        Membership.objects.create(
            user=another_user, group=group, role=Membership.Role.MEMBER
...
class TestKickPermissions:
    def test_kick_as_admin_success(
        self, api_client, group, user, another_user, third_user
    ):
        Membership.objects.create(
            user=another_user, group=group, role=Membership.Role.ADMIN
        )
        Membership.objects.create(
            user=third_user, group=group, role=Membership.Role.MEMBER
        )
        ...
        response = api_client.post(
            f"/api/groups/{group.id}/kick/", {"username": "thirduser"}
        )
        assert response.status_code == 200

    def test_kick_as_member_fails(
        self, api_client, group, user, another_user, member_in_group
    ):
        token = Token.objects.get_or_create(user=member_in_group)[0]
        api_client.credentials(HTTP_AUTHORIZATION=f"Token {token.key}")
        ...
        response = api_client.post(
            f"/api/groups/{group.id}/kick/", {"username": "anotheruser"}
        )
        assert response.status_code == 403

    def test_kick_removes_membership(
        self, authenticated_client, group, user, another_user
    ):
        Membership.objects.create(
            user=another_user, group=group, role=Membership.Role.MEMBER
...
class TestPromotePermissions:
    def test_promote_as_admin_success(
        self, api_client, group, user, another_user, third_user
    ):
        Membership.objects.create(
            user=another_user, group=group, role=Membership.Role.ADMIN
    ...
    def test_promote_as_member_fails(
        self, api_client, group, user, another_user, member_in_group
    ):
        token = Token.objects.get_or_create(user=member_in_group)[0]
        api_client.credentials(HTTP_AUTHORIZATION=f"Token {token.key}")
...
class TestBanPermissions:
    def test_ban_as_head_admin_success(
        self, authenticated_client, group, user, another_user
    ):
        Membership.objects.create(
            user=another_user, group=group, role=Membership.Role.MEMBER
        ...
        response = authenticated_client.post(
            f"/api/groups/{group.id}/ban/", {"username": "anotheruser"}
        )
        assert response.status_code == 200
    ...
    def test_ban_as_admin_fails(self, api_client, group, user, another_user):
        ...
        response = api_client.post(
            f"/api/groups/{group.id}/ban/", {"username": "thirduser"}
        )
        assert response.status_code == 403

    def test_ban_removes_membership_and_invitations(
        self, authenticated_client, group, user, another_user
    ):
        Membership.objects.create(
            user=another_user, group=group, role=Membership.Role.MEMBER
...
class TestInvitationAcceptDecline:
    def test_accept_invitation_success(self, api_client, group, user, another_user):
        ...
        response = api_client.post(
            "/api/groups/accept_invitation/", {"invitation_id": invitation.id}
        )
        assert response.status_code == 200
        assert Membership.objects.filter(user=another_user, group=group).exists()
    ...
    def test_decline_invitation_success(self, api_client, group, user, another_user):
        ...
        response = api_client.post(
            "/api/groups/decline_invitation/", {"invitation_id": invitation.id}
        )
        assert response.status_code == 200
        invitation.refresh_from_db()
