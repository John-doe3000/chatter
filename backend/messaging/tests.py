...
class MessageAPITestCase(TestCase):
    def setUp(self):
        ...
        Membership.objects.create(
            user=self.user1, group=self.group, role=Membership.Role.HEAD_ADMIN
        )
        Membership.objects.create(
            user=self.user2, group=self.group, role=Membership.Role.MEMBER
        )
    ...
    def test_send_text_message(self):
        ...
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="json"
        )
        ...
        self.assertTrue(
            Message.objects.filter(group=self.group, body="Hello, group!").exists()
        )
    ...
    def test_send_message_with_attachment(self):
        ...
        data = {"body": "Check this image", "attachment": image_data, "kind": "image"}
        response = self.client.post(
    ...
    def test_send_attachment_only_message(self):
        ...
        data = {"attachment": audio_data, "kind": "voice"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_poll_messages_with_since(self):
        ...
        msg1 = Message.objects.create(
            group=self.group, sender=self.user1, body="Message 1"
        )
        msg1.created_at = datetime(2024, 1, 1, 10, 0, 0, tzinfo=datetime_timezone.utc)
        msg1.save()

        msg2 = Message.objects.create(
            group=self.group, sender=self.user2, body="Message 2"
        )
        msg2.created_at = datetime(2024, 1, 1, 11, 0, 0, tzinfo=datetime_timezone.utc)
        msg2.save()

        msg3 = Message.objects.create(
            group=self.group, sender=self.user1, body="Message 3"
        )
        msg3.created_at = datetime(2024, 1, 1, 12, 0, 0, tzinfo=datetime_timezone.utc)
        msg3.save()

        # Poll since msg1's creation time
        since = datetime(2024, 1, 1, 10, 0, 0, tzinfo=datetime_timezone.utc).isoformat()
        response = self.client.get(
            f"/api/groups/{self.group.id}/messages/", {"since": since}
        )
    ...
    def test_cursor_pagination(self):
        ...
        for i in range(25):
            Message.objects.create(
                group=self.group, sender=self.user1, body=f"Message {i}"
            )
    ...
    def test_message_includes_attachments(self):
        ...
        message = Message.objects.create(
            group=self.group, sender=self.user1, body="With attachment"
        )
        Attachment.objects.create(
            message=message, file="test.jpg", kind=Attachment.Kind.IMAGE
        )
    ...
    def test_image_attachment_valid_jpeg(self):
        ...
        data = {"body": "JPEG image", "attachment": image_data, "kind": "image"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_image_attachment_valid_png(self):
        ...
        data = {"body": "PNG image", "attachment": image_data, "kind": "image"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_image_attachment_invalid_mime_type(self):
        ...
        data = {"body": "GIF image", "attachment": image_data, "kind": "image"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_image_attachment_oversize(self):
        ...
        data = {"body": "Large image", "attachment": image_data, "kind": "image"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_video_attachment_valid_mp4(self):
        ...
        data = {"body": "MP4 video", "attachment": video_data, "kind": "video"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_video_attachment_invalid_mime_type(self):
        ...
        video_data = self._create_test_file(
            b"fake webm data", "test.webm", "video/webm"
        )

        data = {"body": "WebM video", "attachment": video_data, "kind": "video"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_video_attachment_oversize(self):
        ...
        data = {"body": "Large video", "attachment": video_data, "kind": "video"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_voice_attachment_valid_aac(self):
        ...
        data = {"body": "AAC voice", "attachment": audio_data, "kind": "voice"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_voice_attachment_valid_mp4(self):
        ...
        audio_data = self._create_test_file(
            b"fake mp4 audio data", "test.m4a", "audio/mp4"
        )

        data = {"body": "MP4 voice", "attachment": audio_data, "kind": "voice"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_voice_attachment_valid_mpeg(self):
        ...
        data = {"body": "MPEG voice", "attachment": audio_data, "kind": "voice"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_voice_attachment_invalid_mime_type(self):
        ...
        data = {"body": "WAV voice", "attachment": audio_data, "kind": "voice"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
    ...
    def test_voice_attachment_oversize(self):
        ...
        data = {"body": "Large voice", "attachment": audio_data, "kind": "voice"}
        response = self.client.post(
            f"/api/groups/{self.group.id}/messages/", data, format="multipart"
        )
